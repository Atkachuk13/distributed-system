...
public class Client {
    ...
    private void startSenderThread() {
        Thread t = new Thread(() -> {
            try {
                PrintWriter out = new PrintWriter(masterCommSock.getOutputStream(), true);
                ...
                while (true) {
                    ...
                    // Send to master
                    out.println(msg);
                    System.out.println("Client [" + clientId + "] → Master: " + msg);
                }
        ...
        Thread t = new Thread(() -> {
            try {
                BufferedReader in = new BufferedReader(
                        new InputStreamReader(masterCommSock.getInputStream())
                );
                String line;

                while ((line = in.readLine()) != null) {
                    System.out.println("Master → Client [" + clientId + "]: " + line);
                    ...
                    if (line.startsWith("DONE;")) {
                        ...
                        if (parts.length >= 3) {
                            ...
                            if (doneClientId.equals(clientId)) {
                                System.out.println("Client [" + clientId + "]: Job " + jobId + " is complete!");
                            } else {
